        ...
        for event in pygame.event.get():
            ...
            if board1.curteam == "b": #make AI moves here
                if firstmove == True:
                    ...
                    while True:
                        if board1.board[tile[1]][tile[0]].piece != None and board1.board[tile[1]][tile[0]].piece.color == "b":
                            if moves[tile] != []:
                                move = random.choice(moves[tile])
                                break
                            else:
                                tile = random.choice(list(moves.keys()))
                        else:
                            tile = random.choice(list(moves.keys()))
                    board1.moveTo(board1.board[tile[1]][tile[0]].piece, (tile[0],tile[1]), (move[0],move[1]))
                    play_click()
                    board1.nextturn()
