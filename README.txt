...
Rules:
White begins on the first turn, with turns alternating between black and white after that.
...
Checkmate occurs when the king is in check and there are no other legal moves the side being checked is able to make to break the check. This ends the game, restricting any further movements.
...
The board.py file contains the code for: Board class (methods: self, get_square, draw, makeButton, nextturn, kingLoc, moveTo, 
